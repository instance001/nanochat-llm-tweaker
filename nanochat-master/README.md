# nanochat Local Builder Fork

This folder contains the local builder app that powers the `LLM Tweaker Builder` ChattyCog module.
...
This local builder fork includes upstream-derived nanochat code.

- Upstream nanochat licensing applies where that upstream code applies. See [LICENSE](LICENSE).
- The surrounding ChattyCog wrapper that hosts this app as a module lives one folder up and is documented separately by that wrapper package.

If you are redistributing the full packaged module, keep both the upstream `nanochat-master/LICENSE` file and the wrapper module's `LICENSE` file.
