...
try:
    ...
    wandb = None
from nanochat.common import compute_init, compute_cleanup, print0, DummyWandb, get_base_dir, autodetect_device_type, get_peak_flops, COMPUTE_DTYPE, COMPUTE_DTYPE_REASON, is_ddp_initialized, is_local_only
...
parser.add_argument("--include-identity", type=int, default=1, help="include the published identity dataset in the training mixture (1=yes, 0=no)")
parser.add_argument("--identity-repeats", type=int, default=2, help="number of times to repeat the identity dataset in the training mixture")
...
# wandb logging init
use_dummy_wandb = args.run == "dummy" or not master_process or local_only
if wandb is None:
    use_dummy_wandb = True
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="nanochat-sft", name=args.run, config=user_config)
if local_only and master_process and args.run != "dummy":
    print0("Local-only mode: wandb logging is disabled.")
if wandb is None and master_process:
    print0("wandb is not installed; using local dummy logger.")
if local_only and args.chatcore_every > 0:
    print0("Local-only mode: disabling ChatCORE benchmark eval.")
...
identity_file = args.identity_file or identity_conversations_filepath
...
def parse_file_list(raw: str) -> list[str]:
    ...
    for piece in normalized.split(";"):
        ...
        if candidate:
            items.append(os.path.abspath(os.path.expanduser(candidate)))
    return items

train_filepaths = parse_file_list(args.train_files)
val_filepaths = parse_file_list(args.val_files)
...
if args.include_identity:
    if os.path.exists(identity_file):
        for _ in range(max(1, args.identity_repeats)):
            train_tasks.append(CustomJSON(filepath=identity_file))
        train_sources.append(f"{os.path.basename(identity_file)} x{max(1, args.identity_repeats)}")
    else:
        print0(f"WARNING: identity file not found, skipping it: {identity_file}")

for filepath in train_filepaths:
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"SFT train file not found: {filepath}")
    train_tasks.append(CustomJSON(filepath=filepath))
    train_sources.append(os.path.basename(filepath))

if not train_tasks:
    raise FileNotFoundError(
        "No local SFT training data was found. Provide --train-files and/or publish an identity dataset first."
    )

train_dataset = TaskMixture(train_tasks)
print0(f"Training mixture: {len(train_dataset):,} rows from {', '.join(train_sources)}")

val_tasks = []
val_sources = []
for filepath in val_filepaths:
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"SFT validation file not found: {filepath}")
    val_tasks.append(CustomJSON(filepath=filepath))
    val_sources.append(os.path.basename(filepath))

if not val_tasks and os.path.exists(identity_file):
    val_tasks.append(CustomJSON(filepath=identity_file))
    val_sources.append(os.path.basename(identity_file))
    print0("No validation files provided; using the identity dataset for local smoke-test validation.")

val_dataset = TaskMixture(val_tasks) if val_tasks else train_dataset
if val_sources:
    print0(f"Validation mixture: {len(val_dataset):,} rows from {', '.join(val_sources)}")
elif args.eval_every > 0:
    print0("No validation files provided; validation will reuse the train mixture.")
