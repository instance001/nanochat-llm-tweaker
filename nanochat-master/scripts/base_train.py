...
from nanochat.common import compute_init, compute_cleanup, print0, DummyWandb, print_banner, get_base_dir, autodetect_device_type, get_peak_flops, COMPUTE_DTYPE, COMPUTE_DTYPE_REASON, is_ddp_initialized, is_local_only
...
if local_only and args.core_metric_every > 0:
    print0("Local-only mode: disabling CORE metric because it depends on external evaluation bundles.")
...
train_loader = tokenizing_distributed_data_loader_with_state_bos_bestfit(
    ...
    data_dir=args.corpus_dir or None,
)
...
while True:
    ...
    if steps_done > 0:
        ...
        eta_str = ""
    if "pq_idx" in dataloader_state_dict and "rg_idx" in dataloader_state_dict:
        epoch = f"{dataloader_state_dict['epoch']} pq: {dataloader_state_dict['pq_idx']} rg: {dataloader_state_dict['rg_idx']}"
    else:
        doc_index = dataloader_state_dict.get("doc_index", "?")
        epoch = f"{dataloader_state_dict.get('epoch', '?')} doc: {doc_index}"
