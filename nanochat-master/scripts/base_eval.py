...
from nanochat.common import compute_init, compute_cleanup, print0, get_base_dir, autodetect_device_type, download_file_with_lock, is_local_only
...
def main():
    ...
    if local_only and args.hf_path is not None:
        parser.error("--hf-path is disabled in local-only mode.")
    if local_only and 'core' in eval_modes:
        parser.error("CORE evaluation is disabled in local-only mode because it requires an external bundle.")
